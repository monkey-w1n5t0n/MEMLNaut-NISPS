...
export interface SynthParamDescriptor {
  /** C15 parameter ID */
  id: number;
  /** Stable machine name, matches preset override keys */
  name: string;
  /** Short display label for UI */
  label: string;
  /** Default normalized value [0, 1] */
  defaultValue: number;
  /** True if the underlying parameter is bipolar (centre = 0) */
  bipolar: boolean;
  /** Lower bound for tame-level range constraining */
  safeMin?: number;
  /** Upper bound for tame-level range constraining */
  safeMax?: number;
}

export const SYNTH_PARAM_MAP: SynthParamDescriptor[] = [
  // --- Envelope A (7) ---
...
/** Parameter display names for the param bar UI (one per MLP output index) */
export const SYNTH_PARAM_NAMES: string[] = SYNTH_PARAM_MAP.map(p => p.label);
...
function generateParamColors(): string[] {
...
export const SYNTH_PARAM_COLORS: string[] = generateParamColors();

// ---------------------------------------------------------------------------
// Section metadata (for SynthVisualizer grouping)
// ---------------------------------------------------------------------------

export interface SynthSection {
  /** Section display name */
  name: string;
  /** Number of params in this section */
  count: number;
  /** Representative color (first color of the section's hue range) */
  color: string;
  /** Index of the first param in SYNTH_PARAM_MAP */
  startIndex: number;
}

// Sections in the same order as SYNTH_PARAM_MAP
export const SYNTH_SECTIONS: SynthSection[] = [
  { name: 'Envelope A',          count:  7, color: 'hsl(0, 75%, 55%)',   startIndex:   0 },
  { name: 'Envelope B',          count:  7, color: 'hsl(15, 75%, 55%)',  startIndex:   7 },
  { name: 'Envelope C',          count:  6, color: 'hsl(30, 70%, 55%)',  startIndex:  14 },
  { name: 'Oscillator A',        count:  5, color: 'hsl(35, 85%, 55%)',  startIndex:  20 },
  { name: 'Oscillator B',        count:  5, color: 'hsl(45, 85%, 55%)',  startIndex:  25 },
  { name: 'Shaper A',            count:  6, color: 'hsl(320, 75%, 55%)', startIndex:  30 },
  { name: 'Shaper B',            count:  6, color: 'hsl(340, 70%, 55%)', startIndex:  36 },
  { name: 'Comb Filter',         count:  8, color: 'hsl(180, 70%, 50%)', startIndex:  42 },
  { name: 'State Variable Filter', count: 9, color: 'hsl(210, 75%, 55%)', startIndex: 50 },
  { name: 'Gap Filter',          count:  6, color: 'hsl(165, 65%, 50%)', startIndex:  59 },
  { name: 'Feedback Mixer',      count:  9, color: 'hsl(270, 65%, 55%)', startIndex:  65 },
  { name: 'Output Mixer',        count: 14, color: 'hsl(130, 65%, 50%)', startIndex:  74 },
  { name: 'Cabinet',             count:  8, color: 'hsl(25, 55%, 50%)',  startIndex:  88 },
  { name: 'Flanger',             count: 13, color: 'hsl(300, 60%, 60%)', startIndex:  96 },
  { name: 'Echo',                count:  7, color: 'hsl(195, 70%, 55%)', startIndex: 109 },
  { name: 'Reverb',              count:  6, color: 'hsl(255, 55%, 65%)', startIndex: 116 },
  { name: 'Unison',              count:  3, color: 'hsl(90, 70%, 50%)',  startIndex: 122 },
  { name: 'Mono',                count:  1, color: 'hsl(50, 80%, 55%)',  startIndex: 125 },
];

/**
 * Maps a param name (from SYNTH_PARAM_MAP[i].name) to its section name.
 * Built once at module init.
 */
export const paramToSection: Readonly<Record<string, string>> = (() => {
  const map: Record<string, string> = {};
  let idx = 0;
  for (const section of SYNTH_SECTIONS) {
    for (let i = 0; i < section.count; i++) {
      const param = SYNTH_PARAM_MAP[idx + i];
      if (param) map[param.name] = section.name;
    }
    idx += section.count;
  }
  return map;
 ...
 *
 * @param rawValue  - ML output in [0, 1]
 * @param paramEntry - entry from SYNTH_PARAM_MAP
 * @param tameLevel - 0 (no mitigation) to 1 (strongest)
 * @returns constrained value in [0, 1]
 */
export function applyTame(
  rawValue: number,
  paramEntry: SynthParamDescriptor,
  tameLevel: number,
): number {
 ...
 * @param curveFactor - curve bias in [0, 1]
 * @returns remapped value in [0, 1]
 */
 ...
 *
 * @param rawValue - ML output in [0, 1]
 * @param curve    - curve factor [0, 1], 0.5 = linear
 * @param min      - output range minimum [0, 1]
 * @param max      - output range maximum [0, 1]
