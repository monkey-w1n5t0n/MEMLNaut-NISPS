...
test.describe('URL parameter: ?spread', () => {
    test('?spread=0 does not crash the app', async ({ page }) => {
        ...
        const probeType = await page.evaluate(() => typeof window.__nisps);
    ...
    test('?spread=0.3 is applied as spreadLevel on the ML store', async ({ page }) => {
        ...
        const spreadLevel = await page.evaluate(() => {
            const store = window.__nispsStore;
    ...
    test('?spread=1.5 is clamped to 1 (out-of-range high)', async ({ page }) => {
        ...
        const spreadLevel = await page.evaluate(() => window.__nispsStore.state.spreadLevel);
    ...
    test('?spread=0.3&tame=0.5 combined — app remains functional', async ({ page }) => {
        ...
        const result = await page.evaluate(() => {
            ...
            return {
                ...
                allBounded: probe.getOutputs().every((v) => v >= 0 && v <= 1),
            };
        });
...
test.describe('Fresh load (no stored state)', () => {
    test('starts untrained: 0 examples, null loss', async ({ page }) => {
        ...
        const result = await page.evaluate(() => ({
            exampleCount: window.__nisps.getExampleCount(),
            loss: window.__nisps.getLoss(),
        }));
...
test.describe('probe.saveState() — localStorage format', () => {
    ...
    test('saveState reflects current example count', async ({ page }) => {
        ...
        const result = await page.evaluate((key) => {
            ...
            if (!raw)
                return null;
            return JSON.parse(raw).exampleCount;
        }, STORAGE_KEY);
    ...
    test('saveState reflects current spreadLevel', async ({ page }) => {
        ...
        const result = await page.evaluate((key) => {
            ...
            if (!raw)
                return null;
            return JSON.parse(raw).spreadLevel;
        }, STORAGE_KEY);
...
test.describe('localStorage round-trip', () => {
    test('weights are restored after saveState + reload', async ({ page }) => {
        ...
        const weightsBefore = await page.evaluate((key) => {
            const probe = window.__nisps;
            probe.randomise();
            probe.saveState();
            return Array.from(probe.getWeights());
        }, STORAGE_KEY);
        ...
        const weightsAfter = await page.evaluate(() => Array.from(window.__nisps.getWeights()));
    ...
    test('output mode is restored after saveState + mode switch + reload', async ({ page }) => {
        ...
        await page.goto('/?debug=1');
        await page.waitForFunction(() => window.__nisps !== undefined, { timeout: 20000 });
        const mode = await page.evaluate(() => window.__nispsStore.state.outputMode);
