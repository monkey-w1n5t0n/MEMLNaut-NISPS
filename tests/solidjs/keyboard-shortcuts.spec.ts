...
test.describe('Keyboard shortcuts (useKeyboard hook)', () => {
  ...
  test('key 1 triggers thumbs-down: changes at least one output', async ({ page }) => {
    ...
    const before = await page.evaluate(() => [...(window as any).__nisps.getOutputs()]);
    await page.keyboard.press('1');

    // thumbsDown is synchronous — outputs update immediately
    const after = await page.evaluate(() => [...(window as any).__nisps.getOutputs()]);
    const changed = before.some((v: number, i: number) => Math.abs(v - after[i]) > 0.0001);
  ...
  test('key 2 triggers thumbs-up: increments example count by 1', async ({ page }) => {
    ...
    const before = await page.evaluate(() => (window as any).__nisps.getExampleCount());
    await page.keyboard.press('2');

    // thumbsUp is async — wait for the example to be recorded
    await page.waitForFunction(
      (countBefore: number) => (window as any).__nisps.getExampleCount() > countBefore,
      before,
      { timeout: 5_000 },
    );

    const after = await page.evaluate(() => (window as any).__nisps.getExampleCount());
  ...
  test('key z triggers undo after thumbs-down: weights restored within 1e-4', async ({ page }) => {
    ...
    const weightsBefore = await page.evaluate(() => [...(window as any).__nisps.getWeights()]);

    // thumbs-down via key '1'
    await page.keyboard.press('1');

    // Verify weights changed
    const weightsAfterDown = await page.evaluate(() => [...(window as any).__nisps.getWeights()]);
    const downChanged = weightsBefore.some(
      (v: number, i: number) => Math.abs(v - weightsAfterDown[i]) > 1e-8,
    );
    ...
    const weightsAfterUndo = await page.evaluate(() => [...(window as any).__nisps.getWeights()]);
    let maxDiff = 0;
  ...
  test('Space toggles follow mode on', async ({ page }) => {
    ...
    const initialFollow = await page.evaluate(
      () => (window as any).__nispsInputStore?.followMode?.() ?? false,
    );
  ...
  test('Space toggles follow mode off when already on', async ({ page }) => {
    ...
    const followModeOff = await page.evaluate(
      () => (window as any).__nispsInputStore?.followMode?.() ?? true,
    );
  ...
  test('key 1 does NOT trigger thumbs-down when focus is inside a text input', async ({ page }) => {
    ...
    const changed = outputsBefore.some(
      (v: number, i: number) => Math.abs(v - outputsAfter[i]) > 0.0001,
    );
